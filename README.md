...
- **第0〜11章の解説＋インタラクティブウィジェット**
  - 第0〜4章: 数学基礎（ベクトル・内積・積分）＋反射方程式
  - 第5章: BRDFローブ可視化（完全拡散／光沢／鏡面）
  - 第6章: Lambert（πで割る/割らない比較つき）
  - 第7章: マイクロファセット断面シミュレータ
  - 第8章: D項グラフ＋球体表示。**Beckmann 分布コラム**（GGX比較グラフ・球体トグル）
  - 第9章: フレネルカーブ（F0プリセット）＋リム確認球体
  - 第10章: シャドウイング/マスキング遮蔽シミュレータ＋G関数グラフ
  - 第11章: **ステップ実行計算機**（スライダー→最終RGBの全中間値をフルトレース）

第12章（卒業クイズ・GLSL全文対応）と補足は順次追加予定です（`Cook-TorranceWeb_仕様書.md` 参照）。
